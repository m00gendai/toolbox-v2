...
if(hasReadNewItem != newItem){
    ...
    function generateHelpEntry(tag, title, icon, link){
        const entry = document.createElement("div")
        entry.setAttribute("class", "helpEntry")
        entry.setAttribute("id", `helpEntry_${tag}`)

        const image = document.createElement("img")
        image.setAttribute("class", "helpEntryImage")
        image.setAttribute("src", `./Assets/${icon}`)
        entry.appendChild(image)

        const label = document.createElement("span")
        label.setAttribute("class", "helpEntryLabel")
    ...
    const helpList = [
        {
            ...
            title: "User Manual",
            icon: "manual.png",
            link: "./Manual/Toolbox V2 Guidelines.pdf",
        },
        {
            tag: "readme",
    ...
    document.getElementById("optionsToggle").addEventListener("click", function(){
        ...
        const optionsCcontainer = document.getElementById("optionsContainer")
        const helpCcontainer = document.getElementById("helpContainer")
        menu.style.right = optionsVisible ? "-30rem" : 0
        optionsVisible = !optionsVisible

        for(const entry of optionList){
            if(!document.getElementById(`optionsEntry_${entry.tag}`)){
                optionsCcontainer.appendChild(generateOptionsEntry(entry.tag, entry.title, entry.on, entry.off, entry.func))
            } 
        }
        for(const entry of helpList){
            if(!document.getElementById(`helpEntry_${entry.tag}`)){
                helpCcontainer.appendChild(generateHelpEntry(entry.tag, entry.title, entry.icon, entry.link))
            } 
        }
    })
...
// L E G A L   S T A T E M E N T
    ...
    document.getElementById("legal_statement").addEventListener("click", function(){
        document.getElementById("legalVeil").style.display ="flex";
    })

    document.getElementById("legal_readme").addEventListener("click", function(){
        window.open("./README.md")
    })

    document.getElementById("legal_manual").addEventListener("click", function(){
        window.open("./Manual/Toolbox V2 Guidelines.pdf")
    })
